...
#                                                                             
# PROGRAMMER: Ahmed Mohamed Amin
# DATE CREATED: 15:00 PM 12/15/2020                                 
# REVISED DATE: 
...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    """           
    # Creates dog-names dictionary for quick matching to results_dic labels from
    # real answer & classifier's answer
    names_dic = dict()

    # Reads in dog-names from file, 1 name per line & automatically closes file
    with open(dogfile, "r") as inputfile:
        # Reads in dognames from first line in file
        line = inputfile.readline()

        # Processes each line in file until reaching EOF (end-of-file) by 
        # processing line and adding dog-names to names_dic with while loop
        while line != "":
            ...
            # TODO: 4b. REPLACE pass with CODE to check if the dog-name(line) 
            #          exists within names_dic, then if the dog-name(line) 
            #          doesn't exist within names_dic then add the dog-name(line) 
            #          to names_dic as the 'key' with the 'value' of 1. 
            #
            # adds dog-name(line) to names_dic if it doesn't already exist 
            # in the names_dic dictionary
            if line not in names_dic:
                names_dic.update({line: 1})
    ...
    # How - iterate through results_dic if labels are found in names_dic
    # then label "is a dog" index3/4=1 otherwise index3/4=0 "not a dog"
    for key in results_dic:

        # Pet Image Label IS of Dog (e.g. found in names_dic)
        if results_dic[key][0] in names_dic:
            
            # Classifier Label IS image of Dog (e.g. found in names_dic)
            # appends (1, 1) because both labels are dogs
            if results_dic[key][1] in names_dic:
            ...
            #                              
            # Classifier Label IS NOT image of dog (e.g. NOT in names_dic)
            # appends (1,0) because only pet label is a dog
            else:
                results_dic[key].extend((1, 0))

        # Pet Image Label IS NOT a Dog image (e.g. NOT found in names_dic)
        else:
